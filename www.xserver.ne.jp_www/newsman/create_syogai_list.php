...
$db_name 	= array('tips');
$data_list 	= array("*");
$condition 	= "WHERE category = ? AND show_flag = ? AND find_in_set(?,service_category) AND (status = ? OR DATE_ADD(updated_time, INTERVAL 24 HOUR) > NOW()) ORDER by posted_date DESC, id DESC LIMIT ?";
$value 		= array(4,0,1,'occurred',$number);
...
echo('<p>');
//secure_print($config->comment_shogai);
secure_print('現在、発生中のサーバー障害はございません。');
echo('</p>');
}
}
?>