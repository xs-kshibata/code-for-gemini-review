...
$db_name 	= array('tips');
$data_list 	= array("*");
$condition  = "WHERE category = ? AND show_flag = ? AND find_in_set(?,service_category) AND (status = ? OR DATE_ADD(updated_time, INTERVAL 24 HOUR) > NOW()) ORDER by maintenance_date DESC, id DESC LIMIT ?";
$value 		= array(3,0,1,'plan',$number);
...
secure_print('現在、予定しているメンテナンスはございません。');
echo('</p>');
}
}
?>